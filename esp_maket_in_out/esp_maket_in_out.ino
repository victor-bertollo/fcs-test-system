...
void setAllHiZ() {
  ...
  writeIODIR16(MCP_DRIVER, 0xFFFF);

  // pull-ups off
  writeGPPU16(MCP_DRIVER, 0x0000);

  // preload output latch = 0
  writeGPIO16(MCP_DRIVER, 0x0000);
}
...
void processFrame() {
  ...
  writeGPIO16(MCP_DRIVER, outputValue);
  writeIODIR16(MCP_DRIVER, iodir);

  delayMicroseconds(SETTLE_MICS);

  if (clockMask != 0) {
    // Rising edge: clock LOW -> HIGH.
    writeGPIO16(MCP_DRIVER, outputValue | clockMask);
    delay(CLOCK_HIGH_MS);
